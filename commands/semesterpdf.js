...
bot.onText(/\/getsemester$/, async (msg) => {
  ...
  const docRef = doc(db, "demousers", userId.toString());
...
bot.onText(/\/notes$/, async (msg) => {
   ...
   const userId = msg.from.id; // Get user's Telegram ID
      ...
      try {
          ...
          console.error(error);
          bot.sendMessage(chatId, 'There was an error for fetching the data. Please type /start to setup your data on IIUCbot');
      }
});
