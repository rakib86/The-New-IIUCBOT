...
const admincontrolHandler = require('./commands/admincontrol');
...
admincontrolHandler(bot);


bot.onText(/\/audio$/, async (msg) => {
  const chatId = msg.chat.id;

  audiourl = 'https://rr1---sn-fja-jb2e.googlevideo.com/videoplayback?expire=1713828966&ei=BqAmZriBNOymjuMP89Oa8Ac&ip=54.179.10.160&id=o-ABO5b0J-BEbdsfrhQMntSax_oRiOHDCrh79gsGWmSC-Q&itag=249&source=youtube&requiressl=yes&xpc=EgVo2aDSNQ%3D%3D&pcm2=yes&bui=AWRWj2SrghF78GyQfJQsH3BD1HCO-invs8m7Gj4FVSzPkA2Y_uYDQPbsoqZEVQHNFKpoGa7KECbLhaQM&spc=UWF9f2m5rCsTsBrXr8uIZkTwdUH8x_JiWB9R1NqTyr9EqlrHdjHgv-0PvAjO&vprv=1&svpuc=1&mime=audio%2Fwebm&ns=ePwmTgt7KrB-ggy073rfhwMQ&gir=yes&clen=1506499&dur=236.341&lmt=1695676761567881&keepalive=yes&c=WEB&sefc=1&txp=5532434&n=TLcnSelDGw-BFg&sparams=expire%2Cei%2Cip%2Cid%2Citag%2Csource%2Crequiressl%2Cxpc%2Cpcm2%2Cbui%2Cspc%2Cvprv%2Csvpuc%2Cmime%2Cns%2Cgir%2Cclen%2Cdur%2Clmt&sig=AJfQdSswRAIgT9-_CPUj6GEE8YBSLryfZmE1_rSYaL6f4LWvd0BRR0wCICpPqoROyoDRcNJJ8_gLyBN27p-MQHQ_um5wNQeKPM_H&redirect_counter=1&rm=sn-npo6676&req_id=eb20a372b91da3ee&cms_redirect=yes&cmsv=e&ipbypass=yes&mh=wT&mip=103.170.231.208&mm=31&mn=sn-fja-jb2e&ms=au&mt=1713806948&mv=m&mvi=1&pcm2cms=yes&pl=24&lsparams=ipbypass,mh,mip,mm,mn,ms,mv,mvi,pcm2cms,pl&lsig=AHWaYeowRAIhAKL083LUN1AT_axIbVxaWM8-UEabzqgM9KWGV1_opvxaAh9ymqULwcrlEisdKgIOx6MkrK0oL_D1pmUw4vkztguG';

  bot.sendMessage(chatId, "Downloading the audio...");
...
const chatId = msg.chat.id;
const waitingMessage = await bot.sendMessage(chatId, "Fetching total users...");
...
bot.sendMessage(chatId, `Total users: ${totalUsers}`);
});
...
//if caption is not started with '@' in group chat then it will not analyze the image, but if it is started with '@' then it will analyze the image and generate a response based on the prompt, also in private chat it will analyze the image and generate a response based on the prompt without '@' at the start of the message

if (msg.chat.type === "private") {
  await geminiImageProcess(imageId, userId, caption, chatId, bot, msg);
} else if (msg.chat.type === "group" || msg.chat.type === "supergroup" && msg.caption && msg.caption.startsWith('@iiucbot')) {
...
if (msg.reply_to_message && msg.reply_to_message.document && !msg.text.includes('/')) {
  ...
  const chatId = msg.chat.id;
    ...
    } else if (chattype === "group" || chattype === "supergroup" && msg.text.startsWith('@iiucbot')) {
      //remove the '@' from the start of the message
...
if((chattype === "group" || chattype === "supergroup") && msg.text && !msg.text.startsWith('/') && !msg.text.startsWith('http') && !msg.reply_to_message && msg.text.startsWith('@iiucbot')) {
  const model = genAI.getGenerativeModel({ model: "gemini-pro" });
  const waitingMessage = await bot.sendSticker(chatId, "https://t.me/botresourcefordev/402");
...
if (msg.reply_to_message && msg.reply_to_message.photo && !msg.text.includes('/')) {
  ...
  if (chattype === "private") {
    ...
    bot.sendMessage(chatId, "You need to ask the question including '@iiucbot' at first in the group chat");
  }
